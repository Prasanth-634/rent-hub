...
import { Bell, ShieldCheck, Lock, LogOut, CheckCircle2, Sliders } from 'lucide-react';
...
export function TenantSettings() {
  ...
  return (
    <div className="space-y-6 max-w-3xl mx-auto animate-fade-in">
      <div>
        <h1 className="text-2xl font-bold text-slate-900">Tenant Account Settings</h1>
      ...
      <Card className="p-6 md:p-8 border border-surface-200 shadow-sm space-y-6">
        <div className="flex items-center gap-3 border-b border-surface-100 pb-4">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500">Choose when and how RentVerify sends verification updates.</p>
          </div>
        </div>
        ...
        <div className="space-y-4">
          <label className="flex items-center justify-between cursor-pointer p-3 rounded-xl hover:bg-surface-50 transition-colors">
            <div>
              <span className="text-xs font-bold text-slate-800 block">Verification Request Notifications</span>
              <span className="text-[11px] text-slate-500">Receive email alerts when a landlord or lender requests verification consent.</span>
            </div>
            <input
              type="checkbox"
              checked={emailAlerts}
          ...
          <label className="flex items-center justify-between cursor-pointer p-3 rounded-xl hover:bg-surface-50 transition-colors">
            <div>
              <span className="text-xs font-bold text-slate-800 block">Consent Status Updates</span>
              <span className="text-[11px] text-slate-500">Receive confirmation when your consent approvals or rejections are recorded.</span>
            </div>
            <input
              type="checkbox"
          ...
          <label className="flex items-center justify-between cursor-pointer p-3 rounded-xl hover:bg-surface-50 transition-colors">
            <div>
              <span className="text-xs font-bold text-slate-800 block">Transaction Upload & Report Alerts</span>
              <span className="text-[11px] text-slate-500">Get notified when CSV upload processing completes and reports are generated.</span>
            </div>
            <input
              type="checkbox"
        ...
        <div className="pt-2 flex justify-end">
          <Button variant="primary" onClick={handleSavePreferences} className="bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-semibold px-6">
            Save Preferences
          </Button>
        </div>
      </Card>
      ...
      <Card className="p-6 md:p-8 border border-surface-200 shadow-sm space-y-6">
        <div className="flex items-center gap-3 border-b border-surface-100 pb-4">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500">Manage session security and active tenant logons.</p>
          </div>
        </div>
