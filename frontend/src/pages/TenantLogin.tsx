...
export function TenantLogin() {
  const [email, setEmail] = useState('tenant@rentverify.com');
  const [password, setPassword] = useState('TenantSecret123!');
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      const res = await loginWithGoogle('tenant', idToken);
      if (res.success) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const res = await login('tenant', { email, password });
    if (res.success) {
      navigate('/tenant/dashboard');
    } else {
  ...
  return (
    <div className="min-h-screen bg-surface-50 flex flex-col justify-center py-12 sm:px-6 lg:px-8 animate-fade-in">
      <div className="sm:mx-auto sm:w-full sm:max-w-md text-center">
        <div className="mx-auto h-12 w-12 rounded-2xl bg-emerald-600 flex items-center justify-center text-white font-bold shadow-lg shadow-emerald-500/30">
          <UserCheck className="h-6 w-6" />
        </div>
        <h2 className="mt-4 text-3xl font-extrabold text-slate-900 tracking-tight">Welcome back</h2>
        <p className="mt-2 text-sm text-slate-500">Sign in to manage your rental verification requests.</p>
      </div>
      ...
      <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md">
        <div className="bg-white py-8 px-6 shadow-xl rounded-3xl border border-surface-200 sm:px-10">
          ...
          <form className="space-y-5" onSubmit={handleSubmit}>
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="h-11 w-full rounded-xl border border-surface-200 bg-white pl-10 pr-3 text-sm text-slate-900 outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20"
                />
              </div>
            </div>
            ...
            <Button variant="primary" type="submit" isLoading={isLoading} className="w-full h-11 bg-emerald-600 hover:bg-emerald-700 shadow-lg shadow-emerald-500/25">
          ...
          <div className="mt-6 text-center text-xs text-slate-500">
            ...
            <Link to="/tenant/register" className="font-bold text-emerald-600 hover:underline">
...
export function TenantRegister() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const res = await register('tenant', {
      email,
      password,
      full_name: fullName,
  ...
  return (
    <div className="min-h-screen bg-surface-50 flex flex-col justify-center py-12 sm:px-6 lg:px-8 animate-fade-in">
      <div className="sm:mx-auto sm:w-full sm:max-w-md text-center">
        ...
        <h2 className="mt-4 text-3xl font-extrabold text-slate-900 tracking-tight">Create Tenant Account</h2>
        <p className="mt-2 text-sm text-slate-500">Easily share your verified rental history.</p>
      </div>
      ...
      <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md">
        <div className="bg-white py-8 px-6 shadow-xl rounded-3xl border border-surface-200 sm:px-10">
          ...
          <form className="space-y-4" onSubmit={handleSubmit}>
            <div>
              ...
              <input
                ...
                className="h-10 w-full rounded-xl border border-surface-200 bg-white px-3 text-sm text-slate-900 outline-none focus:border-emerald-500"
              />
            </div>
            ...
            <Button variant="primary" type="submit" isLoading={isLoading} className="w-full h-11 bg-emerald-600 hover:bg-emerald-700 mt-2">
          ...
          <div className="mt-6 text-center text-xs text-slate-500">
            ...
            <Link to="/tenant/login" className="font-bold text-emerald-600 hover:underline">
              Sign In
            </Link>
          </div>
        </div>
